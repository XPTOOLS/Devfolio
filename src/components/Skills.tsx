...
const TechLogo = ({ name }: { name: string }) => {
  ...
  if (!slug) return <div className="w-6 h-6 bg-emerald-500/10 rounded flex items-center justify-center text-[8px] font-black text-emerald-600 uppercase">TI</div>;
    ...
    <div className="py-12">
      <SectionWrapper type="fade-right">
        <h3 className="text-3xl font-black mb-12 flex items-center">
          ...
          <div className="ml-6 h-[2px] bg-emerald-100 dark:bg-emerald-900 flex-grow" />
        </h3>
      </SectionWrapper>
        ...
        {categories.map((category, catIdx) => (
          <div key={category} className="group">
            <SectionWrapper type="fade-left" delay={100}>
              <h4 className="text-xs font-black uppercase tracking-[0.3em] text-emerald-600 dark:text-emerald-400 mb-8 flex items-center">
                {category}
                <span className="ml-4 px-2 py-0.5 bg-emerald-50 dark:bg-emerald-900/50 rounded-lg text-[10px]">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                {SKILLS.filter(s => s.category === category).map((skill, skillIdx) => (
                  <SectionWrapper key={skill.name} type="fade-up" delay={skillIdx * 50}>
                    <div className="p-4 bg-white dark:bg-gray-900 rounded-2xl border-2 border-gray-100 dark:border-gray-800 group/item hover:border-emerald-500 transition-all hover:shadow-lg h-full">
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex-grow bg-gray-100 dark:bg-gray-800 h-1 rounded-full overflow-hidden">
                          <div className="h-full bg-emerald-600 rounded-full transition-all duration-1000" style={{ width: `${skill.level}%` }} />
                        </div>
                        <span className="text-[9px] font-black text-emerald-600 dark:text-emerald-400">{skill.level}%</span>
                      </div>
                    </div>
                  </SectionWrapper>
                ))}
