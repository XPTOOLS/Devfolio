...
const TechIcon = ({ name }: { name: string }) => {
  ...
  return <svg className="w-4 h-4 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg>;
};
...
const ProjectGrid: React.FC<ProjectGridProps> = ({ limit, onProjectClick }) => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'Concept': return 'text-emerald-600 bg-emerald-50 dark:bg-emerald-900/30 border-emerald-200 dark:border-emerald-900/50';
          ...
          <div className="w-full lg:w-auto flex flex-col sm:flex-row gap-5">
            <div className="relative group">
              <input 
                ...
                className="w-full sm:w-72 bg-white dark:bg-gray-900 border-2 border-gray-200 dark:border-gray-800 rounded-2xl px-12 py-4 text-sm font-medium focus:outline-none focus:ring-4 focus:ring-emerald-500/10 shadow-sm transition-all"
              />
              ...
              {categories.map(cat => (
                <button
                  ...
                  className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all whitespace-nowrap ${
                    filter === cat ? 'bg-emerald-600 text-white shadow-xl' : 'text-gray-500 hover:text-emerald-600'
                  }`}
                >
                  {cat}
                </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 sm:gap-16">
        {filteredProjects.map((project: Project, idx: number) => (
          <SectionWrapper key={project.id} type="fade-up" delay={idx * 100}>
            <div 
              className="group relative bg-white dark:bg-gray-900 rounded-[3rem] overflow-hidden border-2 border-gray-200 dark:border-gray-800/50 hover:border-emerald-500/50 transition-all duration-700 transform hover:-translate-y-4 shadow-2xl"
            >
              <div className="aspect-[16/10] overflow-hidden relative">
                <div className="absolute top-6 left-6 z-20 flex flex-col gap-2">
                  <span className="px-5 py-2 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md rounded-full text-[10px] font-black uppercase tracking-[0.2em] text-emerald-600 border-2 border-emerald-100 dark:border-emerald-900/50 shadow-sm w-fit">
                    {project.category}
                  </span>
              ...
              <div className="p-10 sm:p-14 border-t-2 border-gray-200 dark:border-gray-800">
                <h3 className="text-3xl sm:text-4xl font-black mb-6 group-hover:text-emerald-600 transition-colors tracking-tight">
                  {project.title}
                </h3>
                ...
                <div 
                  ...
                  className="w-fit cursor-pointer flex items-center text-[11px] font-black text-emerald-600 uppercase tracking-[0.3em] group/btn"
                >
                  Read More
                  <div className="ml-4 w-12 h-[2px] bg-emerald-100 group-hover/btn:w-20 group-hover/btn:bg-emerald-600 transition-all duration-500" />
                </div>
              </div>
            </div>
          </SectionWrapper>
