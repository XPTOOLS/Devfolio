        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 items-start gap-12 sm:gap-20">
          ...
          <div className="lg:col-span-7 text-center sm:text-left space-y-8 sm:space-y-10 order-2 lg:order-1">
            <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-black tracking-tight leading-[1.1] dark:text-white">
              <div className="flex flex-col lg:flex-row lg:items-center lg:gap-8">
                <div className="lg:flex-1">
                  ...
                  <span className="text-emerald-600 dark:text-emerald-400">Digital Intelligence</span>
                </div>
              </div>
            </h1>
            ...
            <div className="lg:hidden flex justify-center py-6">
              <div 
                className={`w-48 h-48 sm:w-56 sm:h-56 rounded-full overflow-hidden border-4 border-emerald-600 p-1 shadow-2xl transition-all duration-1000 ease-out ${isImageLoaded ? 'scale-100 opacity-100' : 'scale-50 opacity-0'}`}
            ...
            >
              I'm <span className="text-emerald-600 dark:text-emerald-400">{DEV_INFO.name}</span>, a self-learned developer from Burundi.
              I have a passion for web development, app and software development, I love to learn new things. I'm also a tech enthusiast and I love to share my knowledge as well as collaborate with others.
            </div>

            <div className="flex flex-wrap gap-4 sm:gap-6 justify-center sm:justify-start pt-4">
              <button onClick={() => window.dispatchEvent(new CustomEvent('navToProjects'))} className="w-full sm:w-auto px-8 sm:px-10 py-4 sm:py-5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl font-black text-base sm:text-lg flex items-center justify-center gap-3 shadow-xl shadow-emerald-600/30 transition-all hover:scale-105 active:scale-95 uppercase tracking-widest">
                Explore My Work
              </button>
          ...
          <div className="hidden lg:block lg:col-span-5 order-1 lg:order-2">
            <div className="relative h-full flex items-center justify-center">
              <div className="relative w-full max-w-md">
                <div 
                  className={`aspect-square rounded-full border-[12px] border-emerald-600/20 p-4 relative overflow-hidden shadow-[0_0_100px_-20px_rgba(5,150,105,0.3)] transition-all duration-1000 ease-out ${isImageLoaded ? 'scale-100 opacity-100' : 'scale-50 opacity-0'}`}
                  onMouseEnter={() => setIsHovered(true)}
                  onMouseLeave={() => setIsHovered(false)}
                >
                  <img 
                    src={DEV_INFO.photoUrl} 
                    alt={DEV_INFO.name} 
                    className={`w-full h-full object-cover rounded-full border-4 border-emerald-600 transition-transform duration-500 ${isHovered ? 'scale-110' : 'scale-100'}`}
                    onLoad={() => setIsImageLoaded(true)}
                  />
                </div>
                {/* Optional decorative elements */}
                <div className="absolute -top-6 -left-6 w-32 h-32 bg-emerald-500/10 rounded-full blur-3xl"></div>
                <div className="absolute -bottom-6 -right-6 w-32 h-32 bg-emerald-400/10 rounded-full blur-3xl"></div>
              </div>
            </div>
          </div>
        </div>
