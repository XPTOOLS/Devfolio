...
const Layout: React.FC<LayoutProps> = ({ children, activePage, setActivePage }) => {
  ...
  const socialLinks = [
    ...
    { id: 'youtube', url: `${DEV_INFO.youtube}`, icon: <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg> },
  ];
    ...
    <div className="min-h-screen flex flex-col transition-colors duration-300 bg-transparent text-gray-900 dark:text-gray-100 relative overflow-x-hidden">
        ...
        <div className="fixed inset-0 z-[200] bg-white dark:bg-gray-950 flex items-center justify-center">
          <div className="flex flex-col items-center">
            ...
            <div className="w-48 h-1 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
              <div className="h-full bg-emerald-600 animate-[loading_1.5s_ease-in-out_infinite]"></div>
            </div>
          </div>
        </div>
      )}
      ...
      <aside className={`fixed top-0 left-0 z-[70] w-[75vw] h-full bg-white dark:bg-gray-900 border-r-2 border-gray-200 dark:border-gray-800 transition-transform duration-500 lg:hidden rounded-tr-[3.5rem] rounded-br-[3.5rem] shadow-2xl flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-8 h-full flex flex-col overflow-hidden">
          <div className="flex items-center justify-between mb-12 flex-shrink-0">
            ...
            <button onClick={() => setIsSidebarOpen(false)} className="p-2 text-gray-500 hover:text-emerald-600 transition-colors"><svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
          </div>
          <nav className="space-y-2 overflow-y-auto pr-2 scrollbar-hide">
            {navLinks.map(link => (
              <button key={link.id} onClick={() => handleNavClick(link.id)} className={`w-full text-left px-6 py-4 rounded-2xl font-bold flex items-center justify-between transition-all ${isNavActive(link.id) ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-gray-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/20'}`}>
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 px-2 sm:px-6 pt-4 h-20 sm:h-24">
        <div className="max-w-7xl mx-auto h-full bg-white/70 dark:bg-gray-950/70 backdrop-blur-xl border-2 border-gray-200 dark:border-gray-800 rounded-[2rem] shadow-sm flex justify-between items-center px-6 sm:px-10">
          <div className="flex items-center">
            <button onClick={() => setIsSidebarOpen(true)} className="p-2 mr-3 lg:hidden text-gray-500 bg-gray-100 dark:bg-gray-800 rounded-full hover:bg-emerald-50 dark:hover:bg-emerald-900/40 transition-colors"><svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg></button>
          ...
          <div className="hidden lg:flex space-x-8">
            {navLinks.map(link => (
              <button key={link.id} onClick={() => handleNavClick(link.id)} className={`text-[11px] font-black uppercase tracking-[0.2em] relative py-2 flex items-center gap-2 transition-colors ${isNavActive(link.id) ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-500 dark:text-gray-400 hover:text-emerald-600'}`}>
                {link.icon} {link.name}
                {isNavActive(link.id) && <span className="absolute bottom-0 left-0 right-0 h-[3px] bg-emerald-600 rounded-full"></span>}
              </button>
            ))}
          </div>
          <div className="flex items-center space-x-4">
            <button onClick={toggleTheme} className="p-2.5 rounded-xl bg-gray-100 dark:bg-gray-800 text-gray-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/40 transition-colors shadow-sm">
              {theme === 'dark' ? (
            ...
            <nav className="flex items-center space-x-2 text-[11px] text-gray-500 dark:text-gray-400 bg-gray-50/50 dark:bg-gray-900/50 w-fit px-4 py-1.5 rounded-full border-2 border-gray-200 dark:border-gray-800 backdrop-blur-sm shadow-sm transition-all overflow-x-auto whitespace-nowrap scrollbar-hide">
              {getBreadcrumbs()?.map((crumb, idx) => (
                <React.Fragment key={crumb.id}>
                  ...
                  <button onClick={() => handleNavClick(crumb.id)} className={`flex items-center gap-1.5 uppercase tracking-widest transition-colors flex-shrink-0 ${idx === getBreadcrumbs()!.length - 1 || isNavActive(crumb.id) ? 'font-black text-emerald-600 dark:text-emerald-400' : 'font-bold hover:text-emerald-600'}`}>
      ...
      >
        <svg className="absolute w-full h-full -rotate-90 pointer-events-none" viewBox="0 0 50 50">
          ...
          <circle cx="25" cy="25" r={radius} fill="transparent" stroke="currentColor" strokeWidth="3" strokeDasharray={circumference} strokeDashoffset={dashOffset} strokeLinecap="round" className="text-emerald-600 dark:text-emerald-500 transition-all duration-150" />
        </svg>
        <div className="relative flex flex-col items-center justify-center">
            <svg className="w-5 h-5 text-emerald-600 dark:text-emerald-500 group-hover:-translate-y-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 15l7-7 7 7" /></svg>
            <span className="text-[8px] font-black text-emerald-600 dark:text-emerald-500 uppercase mt-0.5">{Math.round(scrollProgress)}%</span>
        </div>
      </button>
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-10">
          <div className="mb-12 text-center md:text-left">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-4">
              {socialLinks.map(s => (
                <a key={s.id} href={s.url} target="_blank" rel="noopener noreferrer" className="p-3 bg-white/50 dark:bg-gray-900/50 border-2 border-gray-100 dark:border-gray-800 rounded-2xl text-gray-500 hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition-all hover:scale-110 shadow-sm" title={s.id}>
                  {s.icon}
                </a>
              ))}
            </div>
          ...
          <div className="pt-8 border-t-2 border-gray-300 dark:border-gray-800 flex flex-col md:flex-row justify-between items-center gap-6">
            <span className="text-gray-500 dark:text-gray-400 text-[10px] font-black uppercase tracking-[0.3em] text-center md:text-left">
              &copy; 2024 - {new Date().getFullYear()} {displayName} <span className="animate-heartbeat inline-block ml-1">&hearts;</span>
            </span>
            <div className="flex gap-8">
              <button onClick={() => handleNavClick('privacy')} className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 hover:text-emerald-500 transition-colors">Privacy</button>
              <button onClick={() => handleNavClick('terms')} className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 hover:text-emerald-500 transition-colors">Terms</button>
            </div>
          </div>
        </div>
      </footer>
