            ...
            <div className="relative z-10 p-8 sm:p-14 h-full flex flex-col justify-between">
              <div>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                  <div className="text-emerald-400 font-black text-[10px] sm:text-xs uppercase tracking-[0.3em]">{exp.company}</div>
                  <div className="text-gray-400 font-black text-[10px] uppercase tracking-[0.2em]">{exp.location}</div>
                </div>
                <h3 className="text-2xl sm:text-5xl font-black tracking-tight text-white mb-6 group-hover:text-emerald-400 transition-colors leading-tight">{exp.position}</h3>
                <div className="inline-block px-5 py-2 bg-emerald-500/20 text-emerald-400 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest border border-emerald-500/30">{exp.duration}</div>
              </div>
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mt-8 gap-6">
                ...
                <div className="hidden sm:block">
                  <div className="w-12 h-1 bg-emerald-500 rounded-full group-hover:w-24 transition-all duration-500"></div>
                </div>
              </div>
            </div>
          </div>
