...
import { FaLightbulb, FaRobot, FaStar } from 'react-icons/fa';
  ...
  return (
    <div className="pb-16 sm:pb-24 overflow-x-hidden">
              ...
              {PHILOSOPHY.map((item, idx) => (
                <SectionWrapper key={idx} delay={idx * 150} type="zoom-in">
                  <div className="bg-white dark:bg-gray-950 p-8 sm:p-10 rounded-[2rem] sm:rounded-[3rem] border-2 border-gray-100 dark:border-gray-800 shadow-sm text-center group transition-all hover:shadow-2xl h-full">
                    <div className="text-4xl sm:text-5xl mb-8 group-hover:scale-125 transition-transform">{iconMap[item.icon]}</div>
      ...
      <SectionWrapper delay={300} type="fade-left">
        <div className="py-16 sm:py-24 max-w-7xl mx-auto px-6 sm:px-10">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-12 sm:mb-16 gap-6">
            ...
            <button onClick={() => window.dispatchEvent(new CustomEvent('navToProjects'))} className="text-emerald-600 font-black uppercase tracking-[0.2em] text-sm hover:translate-x-3 transition-transform flex items-center gap-2 group">
              Full Archive 
      ...
      <SectionWrapper delay={400} type="zoom-out">
        <div className="py-16 sm:py-24 bg-emerald-600/5 dark:bg-emerald-600/10 border-y-2 border-emerald-100 dark:border-emerald-900/50 overflow-hidden relative">
          <div className="max-w-7xl mx-auto px-6 sm:px-10">
              ...
              {[...TESTIMONIALS, ...TESTIMONIALS].map((t, i) => (
                <div key={i} className="flex-shrink-0 w-[300px] sm:w-[450px] flex flex-col items-center text-center px-4 sm:px-8 transition-transform hover:scale-105 duration-500">
                  <div className="relative mb-8">
                    <div className="absolute inset-0 bg-emerald-600 blur-2xl opacity-20 rounded-full scale-150"></div>
                  ...
                  <div className="text-[10px] sm:text-xs text-emerald-600 dark:text-emerald-400 uppercase tracking-[0.3em] mb-8 font-black">{t.position}</div>
