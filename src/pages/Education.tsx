        ...
        <SectionWrapper key={i} delay={i * 200} type="zoom-in">
          <div className="group relative rounded-[2.5rem] sm:rounded-[3rem] overflow-hidden border border-gray-200 dark:border-gray-800 shadow-2xl transition-all hover:scale-[1.01] min-h-[350px] sm:min-h-[400px]">
            ...
            <div className="relative z-10 p-8 sm:p-14 h-full flex flex-col justify-between">
              <div>
                <div className="text-emerald-400 font-black text-[10px] sm:text-xs uppercase mb-4 tracking-[0.3em]">{edu.school}</div>
                <h3 className="text-2xl sm:text-5xl font-black tracking-tight text-white mb-6 group-hover:text-emerald-400 transition-colors leading-tight">{edu.degree}</h3>
                <div className="inline-block px-5 py-2 bg-emerald-500/20 text-emerald-400 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest border border-emerald-500/30">{edu.grade}</div>
              </div>
