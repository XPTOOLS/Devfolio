...
const BlogDetail: React.FC<{ blogId: string, onBack: () => void }> = ({ blogId, onBack }) => {
  ...
  const shareTwitter = () => {
    const text = `Check out "${blog.title}" by Ramy Dev: ${window.location.href}`;
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 sm:px-10 pb-24">
      <SectionWrapper>
        <button 
          onClick={onBack} 
          className="flex items-center text-emerald-600 dark:text-emerald-400 font-black text-xs mb-10 gap-2 hover:underline uppercase tracking-widest transition-all hover:gap-3"
        >
        ...
        <div className="space-y-6 mb-12">
          <div className="flex flex-wrap gap-3">
            {blog.tags.map(t => (
              <span 
                key={t} 
                className="px-4 py-1.5 bg-emerald-50 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400 text-[10px] font-black uppercase tracking-widest rounded-xl border border-emerald-100 dark:border-emerald-800 hover:scale-105 transition-transform"
              >
                {t}
              </span>
            ))}
        ...
        <div 
          className="blog-content prose prose-lg dark:prose-invert max-w-none 
            ...
            prose-blockquote:border-l-4 prose-blockquote:border-emerald-500 prose-blockquote:pl-6 prose-blockquote:italic prose-blockquote:text-gray-600 dark:prose-blockquote:text-gray-400
        ...
        <div className="mt-16 pt-12 border-t-2 border-gray-300 dark:border-gray-800">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={onBack}
              className="px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs uppercase tracking-widest rounded-xl transition-all hover:scale-105"
            >
              Back to posts
            </button>
          </div>
